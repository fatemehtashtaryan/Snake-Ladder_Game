...
public class boardgame : MonoBehaviour
{
    ...
    [SerializeField] private GameObject SnakePrefab_5;
    ...
    void Start()
    {
        ...
        SpawnSnakeRandom(2, 2, 6, 5, 6, SnakePrefab_5);
        SpawnSnakeRandom(2, 2, 5, 5, 4, SnakePrefab_2);
        SpawnSnakeRandom(2, 2, 6, 5, 6, SnakePrefab_4);
        SpawnSnakeRandom(2, 3, 6, 5, 11, SnakePrefab_3);
        ...
        for (int i = 0; i < randomCount_Spacial; i++)
        {
            int rand = GetUniqueRandomIndex(1, Tiles.Count-1, 1);
    ...
    public (RectTransform targetTransform , int targetPosition , bool blackHole) CheckElementsHit(int index){
        foreach (  BoardElementConfig element in elements)
        {
            // Debug.Log(element.getHitInd()+ " d");
            if (element.IsHitIndex(index)){
                if(specials.Contains(index))
                {
                    return (Tiles[element.IsForwardedIndex()].GetTileTransform().GetComponent<RectTransform>(), element.IsForwardedIndex(), true);

                }
                    
                return (Tiles[element.IsForwardedIndex()].GetTileTransform().GetComponent<RectTransform>(), element.IsForwardedIndex(), false);
            }
        }
        return (null, 0, false);
    }
    
    public RectTransform GetTile(int index)
    {
        return Tiles[index].GetTileTransform().GetComponent<RectTransform>();
    }
    ...
    private void SpawnSnakeRandom(int firstRange_i, int firstRange_j, int end_i, int coeff_j, int minus, GameObject SnakePrefab)
    {
        int head;

        do{
            int j = GetUniqueRandomIndex(firstRange_j, 10, 0);
            int i = GetUniqueRandomIndex(firstRange_i, end_i, 0);
