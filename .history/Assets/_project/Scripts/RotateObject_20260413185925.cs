   ...
   public int getFace()
{
    ...
    // اسامی این جهت ها برای نمایش در کنسول
    string[] axisNames = { "+Y (Up)", "-Y (Down)", "+X (Right)", "-X (Left)", "+Z (Forward)", "-Z (Back)" };

    float maxDot = -2f; 
    int bestAxisIndex = 0;

    // ۲. پیدا کردن جهتی که بیشترین همسویی را با آسمان (Vector3.up) دارد
    for (int i = 0; i < 6; i++)
    {
        float dot = Vector3.Dot(localAxes[i], Vector3.up);
        if (dot > maxDot)
        {
            maxDot = dot;
            bestAxisIndex = i; // ایندکس بهترین جهت را ذخیره میکنیم
        }
    }

    // ۳. چاپ جهت در کنسول برای کالیبره کردن
    Debug.Log($"<color=yellow>محوری که الان رو به آسمان است: {axisNames[bestAxisIndex]} (ایندکس: {bestAxisIndex})</color>");
    ...
    {
        case 0: // اگر transform.up رو به بالا بود
            diceValue = 1; // این عدد را بعدا اصلاح میکنید
            break;
        case 1: // اگر -transform.up رو به بالا بود
            diceValue = 2; // این عدد را بعدا اصلاح میکنید
            break;
        case 2: // اگر transform.right رو به بالا بود
            diceValue = 3; // این عدد را بعدا اصلاح میکنید
            break;
        case 3: // اگر -transform.right رو به بالا بود
            diceValue = 4; // این عدد را بعدا اصلاح میکنید
            break;
        case 4: // اگر transform.forward رو به بالا بود
            diceValue = 5; // این عدد را بعدا اصلاح میکنید
            break;
        case 5: // اگر -transform.forward رو به بالا بود
            diceValue = 6; // این عدد را بعدا اصلاح میکنید
            break;
    }
