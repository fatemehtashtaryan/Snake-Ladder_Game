    ...
    public (RectTransform targetTransform , int targetPosition , bool blackHole) CheckElementsHit(int index){
        foreach (  BoardElementConfig element in elements)
        {
            ...
            if (element.IsHitIndex(index)){
                ...
                {
                    return (Tiles[element.IsForwardedIndex()].GetTileTransform(),element.IsForwardedIndex(),true) ;
                }
                    
                return (Tiles[element.IsForwardedIndex()].GetTileTransform(),element.IsForwardedIndex(),false) ;
            }
        }
        return (null, 0, false);
    }
