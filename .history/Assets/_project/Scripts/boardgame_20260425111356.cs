...
public class boardgame : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        SpawnSnakeRandom(2, 5, 6, SnakePrefab);
        SpawnSnakeRandom(2, 4, 6, SnakePrefab_2);
        SpawnSnakeRandom(2, 5, 6, SnakePrefab_4);
        SpawnSnakeRandom(3, 5, 11, SnakePrefab_3);
   ...
   private void SpawnSnakeRandom(int firstRange, int coeff, int minus, GameObject SnakePrefab)
{
    Debug.Log("=== SpawnSnakeRandom DEBUG START ===");

    int head;

    do
    {
        int j = GetUniqueRandomIndex(firstRange, 10, 0);
        int i = GetUniqueRandomIndex(2, 5, 0);
        head = i + j * coeff;

        Debug.Log($"Generated head = {head}");
    }
    while (ladders_index.Contains(head));

    int tale = head - minus;
    Debug.Log($"Calculated tale = {tale}");

    // ---------- Debug لیست‌ها ----------
    Debug.Log("orderedTile == null ? " + (orderedTile == null));
    Debug.Log("orderedTile Count = " + orderedTile.Count);

    if (tale < 0 || tale >= orderedTile.Count)
        Debug.LogError($"❌ ERROR: tale index OUT OF RANGE → tale={tale}, Count={orderedTile.Count}");

    if (head < 0 || head >= orderedTile.Count)
        Debug.LogError($"❌ ERROR: head index OUT OF RANGE → head={head}, Count={orderedTile.Count}");

    Tile tailTile = orderedTile[tale];
    Tile headTile = orderedTile[head];

    Debug.Log("tailTile null? " + (tailTile == null));
    Debug.Log("headTile null? " + (headTile == null));

    if (tailTile == null || headTile == null)
        Debug.LogError("❌ ERROR: One of the Tile references is NULL!!");

    // ---------- Debug Prefab ----------
    Debug.Log("SnakePrefab null? " + (SnakePrefab == null));

    // ---------- Debug Transform ----------
    Transform spawnTransform = tailTile.GetSnakeSpawnTransform();
    Debug.Log("spawnTransform null? " + (spawnTransform == null));

    if (spawnTransform == null)
        Debug.LogError("❌ ERROR: tailTile.GetSnakeSpawnTransform() returned NULL!");

    // ---------- Instantiate ----------
    GameObject SpawnSnake = Instantiate(SnakePrefab, spawnTransform);
    Debug.Log("Spawned Snake? " + (SpawnSnake != null));

    // ---------- Config script ----------
    SnakeConfig config = SpawnSnake.GetComponent<SnakeConfig>();
    Debug.Log("SnakeConfig null? " + (config == null));

    if (config == null)
        Debug.LogError("❌ ERROR: Prefab has NO SnakeConfig component!");

    // ---------- Final Initialize ----------
    int hitIndex = headTile.GetIndex();
    int forwardedIndex = tailTile.GetIndex();

    Debug.Log($"Initialize Snake → hitIndex={hitIndex}, forwardedIndex={forwardedIndex}");

    config.Initialize(hitIndex, forwardedIndex);

    elements.Add(config);

    Debug.Log("=== SpawnSnakeRandom DEBUG END ===");
}
