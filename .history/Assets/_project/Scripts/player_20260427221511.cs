...
public class player : MonoBehaviour
{
    ...
    IEnumerator MovePawnSteps()
    {
        ...
        while (somethingHappened)
        {
            ...
            yield return StartCoroutine(CheckForHitRoutine(result => hitHappened = result));
    ...
    private IEnumerator checkTile_situation(int targetPos)
    {
        ...
        foreach (var p in GameManager.Instance.players)
        {
            if (p.Pawn == this.Pawn)
                continue;
            if (p.Pawn.GetPos() == this.Pawn.GetPos())
            {
    ...
    private IEnumerator CheckForHitRoutine(System.Action<bool> result)
    {
        ...
        if (res.targetTransform != null)
        {
            ...
            if (res.blackHole)
                yield return StartCoroutine(Pawn.MoveStep_blackHole(res.targetTransform, res.targetPosition));
            else
                yield return StartCoroutine(Pawn.MoveStep(res.targetTransform, res.targetPosition));
        }
        // if (res.targetTransform != null)
        // {
