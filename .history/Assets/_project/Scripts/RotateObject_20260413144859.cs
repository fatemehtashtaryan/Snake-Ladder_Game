...
public class RotateObject : MonoBehaviour
{
    ...
    public int getFace()
    {
        // نکته: استفاده از eulerAngles برای تشخیص عدد تاس کمی خطرناک است 
        // چون ممکنه زاویه 270 را -90 نشان دهد.
        // پیشنهاد می‌شود از روش Vector3.Dot استفاده کنید، اما فعلا کد خودتان را اصلاح کردم:
        
        float normalizedX = Mathf.Repeat(x, 360f);
        float normalizedY = Mathf.Repeat(y, 360f);
        float normalizedZ = Mathf.Repeat(z, 360f);

        // اینجا منطق تشخیص عدد بر اساس زوایای خاص خودتان را قرار دهید
        // مثال:
        if (normalizedX == 0 && (normalizedY == 270 || normalizedY == -90)) return 1;
        // ... بقیه شرط ها
